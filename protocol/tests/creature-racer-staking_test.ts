...
function getUserShare(chain: Chain, user: Identity) {
  ...
  const share = 
      chain.callReadOnlyFn('creature-racer-staking-v3',
                           'get-user-share',
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let b1 = chain.mineBlock([
      Tx.contractCall('creature-racer-staking-v3', 'enter-staking',
                      [ types.uint(1) ],
                      uA.address)
    ]);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let scRes = chain.callReadOnlyFn('creature-racer-staking-v3',
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
      ...
      let scRes2 = chain.callReadOnlyFn('creature-racer-staking-v3',
                                        'get-staked-creatures',
                                        [ types.principal(uA.address),
                                          types.uint(1) ],
                                        uA.address);
