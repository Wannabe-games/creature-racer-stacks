...
function getInvitationsByInvitee(chain: Chain,
                                   user: Account,
                                   operator: Account) {
  return chain.callReadOnlyFn('creature-racer-referral-nft-v2',
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const getPercentageOfRewardBPS = (user: Account) => {
      const res = chain.callReadOnlyFn('creature-racer-referral-nft-v2',
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let res = chain.callReadOnlyFn('creature-racer-referral-nft-v2',
                                   'royalty-info',
                                   [types.uint(1),
                                    types.uint(10000)],
                                   user1.address);
