...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const rewardPoolAddress = owner.address + '.creature-racer-reward-pool';

    setOperator(chain, owner, operator);

    let b1 = chain.mineBlock([
      Tx.contractCall('creature-racer-payment',
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const paymentContractAddress = owner.address + '.creature-racer-payment';
    const rewardPoolAddress = owner.address + '.creature-racer-reward-pool';
    const referralPoolAddress = owner.address + '.creature-racer-referral-pool';
