...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const rewardPoolAddress = owner.address + '.creature-racer-reward-pool-v2';

    setOperator(chain, owner, operator);

    let b1 = chain.mineBlock([
      Tx.contractCall('creature-racer-payment-v2',
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const paymentContractAddress = owner.address + '.creature-racer-payment-v2';
    const rewardPoolAddress = owner.address + '.creature-racer-reward-pool-v2';
    const referralPoolAddress = owner.address + '.creature-racer-referral-pool-v2';
